...
public class negocioConfirmacion
{
    ...
    public Int64 setInsertClienteConsulta(Int64 id_cita, Int64 id_cliente)
    {
        ...
        if (objDatos.updateTipoCliente(id_cliente, 1) >= 1)
        {
            //cambiar estado a cita por que fue confirmada
            if (objDatos.updateEstadoConfirmadaCita(id_cita, 1) >= 1)
            {
    ...
    public Int16 setEliminarClienteCita(Int64 id_cita, Int64 id_cliente)
    {
        Int16 id = 0;
        //cambiar el tipo cliente de cliente a eliminado estado 3
        if (objDatos.updateTipoCliente(id_cliente, 3) >= 1)
        {
            //cambiar estado a cita por que fue eliminada
            if (objDatos.updateEstadoConfirmadaCita(id_cita, 3) >= 1)
            {
                id = 1;
            }
        }
